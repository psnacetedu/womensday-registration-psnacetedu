...
document.getElementById("faculty-sports").innerHTML = facultySports.map(event => `<a href="events/students/App Development.html" style="text-decoration:none;" onclick="setVariable('${event.name}','${event.time}','${event.contact}','${event.date}','${event.head}');"><li>${event.name}</li></a>`).join('');
